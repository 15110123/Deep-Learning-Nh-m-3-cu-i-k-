...
def main(config):
    ...
    if config.dataset in ['FEI', 'Both']:
        fei_loader = get_loader(config.fei_image_dir, None, None,
                                 config.fei_crop_size, config.image_size, config.batch_size,
                                 'FEI', config.mode, config.num_workers)
    

    # Solver for training and testing StarGAN.
    solver = Solver(celeba_loader, fei_loader, config)

    if config.mode == 'train':
        if config.dataset in ['CelebA', 'FEI']:
            solver.train()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--c2_dim', type=int, default=2, help='dimension of domain labels (2nd dataset)')
    parser.add_argument('--celeba_crop_size', type=int, default=178, help='crop size for the CelebA dataset')
    parser.add_argument('--fei_crop_size', type=int, default=256, help='crop size for the FEI dataset')
    ...
    parser.add_argument('--fei_image_dir', type=str, default='data/FEIFace/train')
